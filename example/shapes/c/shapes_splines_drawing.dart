...
void main()
{
  ...
  while (!WindowShouldClose())
  {
    ...
    if (IsMouseButtonPressed(MouseButton.MOUSE_BUTTON_RIGHT.value) && (pointCount < MAX_SPLINE_POINTS))
    {
    ...
    for (int i = 0; i < pointCount; i++)
    {
      ...
      {
        focusedPoint = i;
        if (IsMouseButtonDown(MouseButton.MOUSE_BUTTON_LEFT.value)) selectedPoint = i; 
        break;
      }
      else focusedPoint = -1;
    }
    ...
    if (selectedPoint >= 0)
    {
      ...
      if (IsMouseButtonReleased(MouseButton.MOUSE_BUTTON_LEFT.value)) selectedPoint = -1;
    }
    ...
    if ((splineTypeActive == .SPLINE_BEZIER) && (focusedPoint == -1))
    {
      ...
      {
        if (CheckCollisionPointCircle(mousePos, control[i].start.ref, 6.0))
        {
          focusedControlPoint = control[i].start;
          if (IsMouseButtonDown(MouseButton.MOUSE_BUTTON_LEFT.value)) selectedControlPoint = control[i].start; 
          break;
        }
        else if (CheckCollisionPointCircle(mousePos, control[i].end.ref, 6.0))
        {
          focusedControlPoint = control[i].end;
          if (IsMouseButtonDown(MouseButton.MOUSE_BUTTON_LEFT.value)) selectedControlPoint = control[i].end; 
          break;
        }
      ...
      if (selectedControlPoint != nullptr)
      {
        ...
        if (IsMouseButtonReleased(MouseButton.MOUSE_BUTTON_LEFT.value)) selectedControlPoint = nullptr;
      }
    }
    
    if (IsKeyPressed(KeyboardKey.KEY_ONE.value)) splineTypeActive = .SPLINE_LINEAR;
    else if (IsKeyPressed(KeyboardKey.KEY_TWO.value)) splineTypeActive = .SPLINE_BASIS;
    else if (IsKeyPressed(KeyboardKey.KEY_THREE.value)) splineTypeActive = .SPLINE_CATMULLROM;
    else if (IsKeyPressed(KeyboardKey.KEY_FOUR.value)) splineTypeActive = .SPLINE_BEZIER;
    ...
    BeginDrawing();
      ...
      {
        DrawSplineLinear(points, pointCount, splineThickness.value, RED);
      }
      else if (splineTypeActive == .SPLINE_BASIS)
      {
        DrawSplineBasis(points, pointCount, splineThickness.value, RED);
      ...
      {
        DrawSplineCatmullRom(points, pointCount, splineThickness.value, RED);
      ...
      else if (splineTypeActive == .SPLINE_BEZIER)
      {
        ...
        DrawSplineBezierCubic(pointsInterleaved, 3*(pointCount - 1) + 1, splineThickness.value, RED);
        ...
        */
        ...
        for (int i = 0; i < pointCount - 1; i++)
        {
          ...
          DrawCircleV(p.start.ref, 6, GOLD);
          DrawCircleV(p.end.ref, 6, GOLD);
          if (focusedControlPoint == p.start) DrawCircleV(p.start.ref, 8, GREEN);
          else if (focusedControlPoint == p.end) DrawCircleV(p.end.ref, 8, GREEN);
          DrawLineEx(points[i], p.start.ref, 1.0, LIGHTGRAY);
          DrawLineEx(points[i + 1], p.end.ref, 1.0, LIGHTGRAY);
      
          DrawLineV(points[i], p.start.ref, GRAY);
          // DrawLineV(p.start.ref, p.end.ref, LIGHTGRAY);
          DrawLineV(p.end.ref, points[i + 1], GRAY);
        }
      }
