...
void main()
{
  ...
  final sinePoints = Vector2$.AtUnique(count: WAVE_POINTS);
  final cosPoints = Vector2$.AtUnique(count: WAVE_POINTS);
  final center = Vector2$.AtUnique().set((screenWidth/2.0) - 30.0, screenHeight/2.0);
  final start = Rectangle$.AtUnique().set(20.0, screenHeight - 120.0, 200.0, 100.0);
  double radius = 130.0;
  final angle = Float32$.ValueUnique(0.0);
  ...
  while (!WindowShouldClose())
  {
    ...
    final point = Vector2$.At('point').set(center.ref.x + cosRad*radius, center.ref.y - sinRad*radius);
    final limitMin = Vector2$.At('limitMin').set(center.ref.x - radius, center.ref.y - radius);
    final limitMax = Vector2$.At('limitMax').set(center.ref.x + radius, center.ref.y + radius);
    ...
    final tangentPoint = Vector2$.At('tangentPoint').set(center.ref.x + radius, center.ref.y - tangent*radius);
    final cotangentPoint = Vector2$.At('cotangentPoint').set(center.ref.x + cotangent*radius, center.ref.y - radius);
    ...
    BeginDrawing();
      ...
      // Cotangent (orange)
      DrawLineEx(Vector2$.$1.set(center.ref.x, limitMin.ref.y), Vector2$.$2.set(cotangentPoint.ref.x, limitMin.ref.y), 2.0, ORANGE);
      ...
      DrawLineEx(Vector2$.$1.set(center.ref.x, limitMin.ref.y), Vector2$.$2.set(center.ref.x, limitMax.ref.y), 1.0, GRAY);
      DrawLineEx(Vector2$.$1.set(limitMin.ref.x, center.ref.y), Vector2$.$2.set(limitMax.ref.x, center.ref.y), 1.0, GRAY);

      // Wave graph axes
      DrawLineEx(Vector2$.$1.set(start.ref.x, start.ref.y), Vector2$.$2.set(start.ref.x, start.ref.y + start.ref.height), 2.0, GRAY);
      DrawLineEx(Vector2$.$1.set(start.ref.x + start.ref.width, start.ref.y), Vector2$.$2.set(start.ref.x + start.ref.width, start.ref.y + start.ref.height), 2.0, GRAY);
      DrawLineEx(Vector2$.$1.set(start.ref.x, start.ref.y + start.ref.height/2), Vector2$.$2.set(start.ref.x + start.ref.width, start.ref.y + start.ref.height/2), 2.0, GRAY);
      ...
      // Sine (red - vertical)
      DrawLineEx(Vector2$.$1.set(center.ref.x, center.ref.y), Vector2$.$2.set(center.ref.x, point.ref.y), 2.0, RED);
      DrawLineDashed(Vector2$.$1.set(point.ref.x, center.ref.y), Vector2$.$2.set(point.ref.x, point.ref.y), 10, 4, RED);
      DrawText("Sine ${sinRad.f2}".toC, 640, 190, 6, RED);
      DrawCircleV(Vector2$.$1.set(start.ref.x + (angle.value/360.0)*start.ref.width, start.ref.y + ((-sinRad + 1)*start.ref.height/2.0)), 4.0, RED);
      DrawSplineLinear(sinePoints, WAVE_POINTS, 1.0, RED);

      // Cosine (blue - horizontal)
      DrawLineEx(Vector2$.$1.set(center.ref.x, center.ref.y), Vector2$.$2.set(point.ref.x, center.ref.y), 2.0, BLUE);
      DrawLineDashed(Vector2$.$1.set(center.ref.x, point.ref.y), Vector2$.$2.set(point.ref.x, point.ref.y), 10, 4, BLUE);
      DrawText("Cosine ${cosRad.f2}".toC, 640, 210, 6, BLUE);
      DrawCircleV(Vector2$.$1.set(start.ref.x + (angle.value/360.0)*start.ref.width, start.ref.y + ((-cosRad + 1)*start.ref.height/2.0)), 4.0, BLUE);
      DrawSplineLinear(cosPoints, WAVE_POINTS, 1.0, BLUE);

      // Tangent (purple)
      DrawLineEx(Vector2$.$1.set(limitMax.ref.x, center.ref.y), Vector2$.$2.set(limitMax.ref.x, tangentPoint.ref.y), 2.0, PURPLE);
      ...
      DrawLineEx(Vector2$.$1.set(center.ref.x, center.ref.y), point.ref, 2.0, BLACK);
      ...
      GuiToggle(Rectangle$.$1.set(640, 70, 120, 20), "Pause".toC, pause);
      GuiSetStyle(GuiControl.LABEL.value, GuiControlProperty.TEXT_COLOR_NORMAL.value, ColorToInt(LIME));
      GuiSliderBar(Rectangle$.$1.set(640, 40, 120, 20), "Angle".toC, "${angle.value.f0}°".toC, angle, 0.0, 360.0);
      GuiGroupBox(Rectangle$.$1.set(620, 110, 140, 170), "Angle Values".toC);
