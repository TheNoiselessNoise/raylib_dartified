  ...
  while (!rl.Core.WindowShouldClose())
  {
    ...
    rl.Core.BeginDrawing();
      ...
      if (!angleMode)
      {
        ...
        rl.Core.DrawLineEx(startPos.ref, endPos.setD(v1).ref, 2.0, rl.C.BLACK);
        rl.Core.DrawLineEx(startPos.ref, endPos.setD(v2).ref, 2.0, rl.C.RED);
      ...
      else if (angleMode)
      {
        ...
        rl.Core.DrawLine(0, screenHeight~/2, screenWidth, screenHeight~/2, rl.C.LIGHTGRAY);
        rl.Core.DrawLineEx(startPos.ref, endPos.setD(v2).ref, 2.0, rl.C.RED);
      ...
      rl.Core.DrawText("v0".toC, v0.x.toInt(), v0.y.toInt(), 10, rl.C.DARKGRAY);

      if (!angleMode && v0.sub(v1).y > 0.0)
        rl.Core.DrawText("v1".toC, v1.x.toInt(), (v1.y-10).toInt(), 10, rl.C.DARKGRAY);
      if (!angleMode && v0.sub(v1).y < 0.0)
        rl.Core.DrawText("v1".toC, v1.x.toInt(), v1.y.toInt(), 10, rl.C.DARKGRAY);

      if (angleMode)
        rl.Core.DrawText("v1".toC, (v0.x + 40).toInt(), v0.y.toInt(), 10, rl.C.DARKGRAY);

      rl.Core.DrawText("v2".toC, (v2.x-10).toInt(), (v2.y-10).toInt(), 10, rl.C.DARKGRAY);
