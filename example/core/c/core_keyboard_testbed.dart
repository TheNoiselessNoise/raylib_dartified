...
// Run it: dart run core_keyboard_testbed.dart
import '../../base_c.dart';
...
void main() {
  ...
  InitWindow(screenWidth, screenHeight, "core_keyboard_testbed".toC);
  SetTargetFPS(60);
  ...
  while (!WindowShouldClose()) {
    ...
    if (key > 0) TraceLog(
      ...
      "KEYBOARD TESTBED: KEY PRESSED:    $key".toC
    );

    int ch = GetCharPressed();
    if (ch > 0) TraceLog(
      TraceLogLevel.LOG_INFO.value,
      "KEYBOARD TESTBED: CHAR PRESSED:   ${String.fromCharCode(ch)} ($ch)".toC
    );
    ...
    BeginDrawing();
      ...
      DrawText("KEYBOARD LAYOUT: ENG-US".toC, 26, 38, 20, LIGHTGRAY);
...
void GuiKeyboardKey(RectangleC bounds, int key)
{
  ...
  else
  {
    if (IsKeyDown(key))
    {
      ...
      DrawText(GetKeyText(key).toC, (bounds.x + 4).toInt(), (bounds.y + 4).toInt(), 10, MAROON);
    }
    else
    {
      DrawRectangleLinesEx(bounds, 2.0, DARKGRAY);
      DrawText(GetKeyText(key).toC, (bounds.x + 4).toInt(), (bounds.y + 4).toInt(), 10, DARKGRAY);
    }
  }
